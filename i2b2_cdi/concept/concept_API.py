...
def getConcept(request, crc_ds):
    with crc_ds as cursor:
        if request.query_string:
            ...
            if hpath:
                hpath = formatPath(hpath)#.replace("\\","\\\\")
                cpath = humanPathToCodedPath(crc_ds.database, hpath) 
            cpath = formatPath(cpath).replace("\\","\\\\")
            if(crc_ds.dbType=='pg'):   
                query = "SELECT case when cd.concept_type = 'float' then 'NUMERIC' else 'TEXTUAL'end as type,cd.description,cd.concept_path,concept_cd,cd.update_date,cd.unit_cd,cd.concept_blob, cd.name_char FROM "+crc_ds.database+".concept_dimension as cd  where cd.concept_path ilike %s"
            ...
            if(crc_ds.dbType=='pg'):
                query = "SELECT case when cd.concept_type = 'float' then  'NUMERIC' else 'TEXTUAL' end as type,cd.description,cd.concept_path,concept_cd,cd.update_date,cd.unit_cd,cd.concept_blob, cd.name_char FROM "+crc_ds.database+".concept_dimension as cd "
            if(crc_ds.dbType=='mssql'):
    ...
    concept_list = list(df.transpose().to_dict().values())
    response = (jsonify(concept_list), 200)
    return response
...
def deleteConcept(request, crc_ds, ont_ds,cpath=None):
    try:
        if cpath is None: #.query_string:
            ...
            hpath = request.args.get('hpath')
            if hpath:
                cpath = humanPathToCodedPath(crc_ds.database, formatPath(hpath))
        ...
        if validateConcept(crc_ds.database, cpath):
            ...
            if (rowcount > 0):
                ...
                response = ("No concept found with path like "+cpath, 404)
        else:
...
def generate_load_csv(data, crc_db, ont_db):
    try:
        ...
        try:
            if blob is not None:
                if 'positive_path' in blob:

                    positive_paths = blob['positive_path'] 
                    negative_paths = blob['negative_path'] 
                    formatted_positive_paths = [ formatPath(path)  for path in positive_paths]
                    formatted_negative_paths = [ formatPath(path)  for path in negative_paths]

                    formatted_blob = {
                    'positive_path' : formatted_positive_paths,
                    'negative_path' : formatted_negative_paths,
                    'time_buffer' : blob['time_buffer'],
                    'feature_selection_count' : blob['feature_selection_count']
                    }
                elif 'positive_patient_set' in blob:
                    formatted_blob = blob
        ...
        row = [['type','unit','path','name','code','description','blob'],[concept_type,unit,formatted_path,name,code,description,formatted_blob]]
        now = datetime.now()
