...
from i2b2_cdi.ML.mlHelper import serialize_model_to_base64, clean_json_string, convert_numpy_types, get_tuple_from_df, format_data_paths,get_installed_packages, get_top_n_features, load_tuple
...
#%%
    cursor.execute('DROP TABLE IF EXISTS bufferred_patient_facts;')
    bufferred_patient_facts_sql = sql.SQL(f"""      
        ...
        where f.start_date<= b.buffer_cut_off  and
         concept_cd in (
            SELECT concept_cd FROM codes_data)                
        """)
